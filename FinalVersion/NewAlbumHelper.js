    ...
    createNewAlbum : function(component)
    {
        ...
        action.setParams(
            {"album" : component.get("v.newAlbum"),
             "author" : component.get("v.author.Name") });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (component.isValid() && state == "SUCCESS") {
                console.log("Album successfully inserted.");
                var appEvent = $A.get("e.c:AddToAlbumsList");
                appEvent.setParams({"album" : response.getReturnValue()});
                appEvent.fire();
                console.log("AddToAlbumsList event fired!");
            } else if (state === "ERROR") {
